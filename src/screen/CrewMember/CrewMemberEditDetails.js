...
class CrewMemberEditDetails extends React.Component {
  constructor() {
    super();
    this.state = {
      userFName: '',
  ...
  componentDidMount = async () => {
    ...
    first_name = await getAsyncStorage('CrewMemberFName');
    last_name = await getAsyncStorage('CrewMemberLName');
  ...
  handleSubmitPress = async () => {
    ...
    if (String(this.state.userNumber).length < 10) {
    ...
    if (responseJson.status == true) {
      ...
      await setAsyncStorage('CrewMemberFName', this.state.userFName);
      await setAsyncStorage('CrewMemberLName', this.state.userLName);
      await setAsyncStorage('CrewMemberName', userFullName);
      await setAsyncStorage('CrewMemberPhone', this.state.userNumber);
      this.props.navigation.replace('CrewMemberProfile');
    } else {
      hideLoader();
...
export default CrewMemberEditDetails;
